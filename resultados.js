(function(){
  ...
  /* ========= Ajustes visibles ========= */
  const TOTAL_RESPONSES = 180; // contador principal
  const BAR_CONFIG = [
    { max: 82, target: 71 }, // primero
    { max: 65, target: 58 }  // segundo
  ];

  /* ========= Catálogos y exámenes (copiados del calendario para consistencia) ========= */
  const DAY_NAMES = ["L","M","X","J","V","S","D"];
  const MONTH_NAMES = ["Noviembre","Diciembre","Enero","Febrero","Marzo","Abril","Mayo","Junio"];
  ...
  /* ========= Construcción calendario (solo lectura) ========= */
  function monthList(){
  ...
  function buildCalendars(container){
    ...
    monthList().forEach(({y,m})=>{
      ...
      const t=document.createElement("h3"); t.textContent=["Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre"][m]+" "+y;
      h.appendChild(t); sec.appendChild(h);
  ...
  function canonicalKeyFor(year, proposals){
    // ordena por exam id fijo del año
  ...
  function renderFullCalendar(year, cluster, altCluster){
    ...
    // diferencias respecto al alterno en fantasma (si existe)
    if(altCluster){
      for(const ex of EXAMS_BY_YEAR[year]){
  ...
  function animateBars(){
    qsa(".progress").forEach((p, i)=>{
      const max = Number(p.dataset.max||0);
      const target = Number(p.dataset.target||0);
      p.querySelector(".fill").style.width = Math.max(0, Math.min(100, (100*target/max))) + "%";
      const meta = p.parentElement.querySelector(".progress-meta .current");
      if(meta){
        let cur=0;
        const dur = p.querySelector(".fill").classList.contains("slow") ? 3000 : 2000;
        const t0 = performance.now();
        function step(now){
          const k = Math.min(1, (now - t0)/dur);
  ...
  function attachUI(){
    ...
    qsa('input[name="yr"]').forEach(r=>{
      r.addEventListener("change", ()=>{
        ...
        // reset view to moda por examen al cambiar
